...
📌 Team update (2026-08-28T16:51:06.599+02:00): Request-side header attributes are now explicitly named OpenApiRequestHeaderParameter* for symmetry with response headers, and all four reusable header attributes also ship generic Attribute<T> variants so reusable definitions/collections can use compile-time generic syntax instead of typeof(...) while keeping the existing non-generic forms — decided by Lead, Functions, Tester.

📌 Team update (2026-08-31T11:40:04.333+02:00): Lead added `IOpenApiResponseDefinition` plus `OpenApiResponseAttribute<T>`, kept the non-generic response attribute backward compatible, and approved the finished batch after clean build verification and 143/143 passing tests — decided by Lead, Tester, Functions.
